...
class UnitTests(unittest.TestCase):
    ...
    def test_get_passage_list_from_string(self):
        text = bible_extractor.get_passage('1 John 1:8 - 9', output_as_list=True)
    ...
    def test_get_passage_list_from_string_empty(self):
        text = bible_extractor.get_passage('', output_as_list=True)
        self.assertEqual([], text, 'Passage is incorrect')

    def test_get_passage_list_from_string_without_passage_numbers(self):
        text = bible_extractor.get_passage('Haggai 1:3 - 4', show_passage_numbers=False, output_as_list=True)
    ...
    def test_get_passage_list_from_string_nlt(self):
        text = bible_extractor.get_passage('1 John 1:8 - 9', translation='NLT', output_as_list=True)
    ...
    def test_get_passage_list(self):
        text = bible_extractor.get_online_passage('1 John', 1, 8, output_as_list=True)
        john = ['\u2078 If we claim to be without sin, we deceive ourselves and the truth is not in us.']
        self.assertEqual(john, text, 'Passage is incorrect')

    def test_get_passages_list(self):
        text = bible_extractor.get_online_passages('1 John', 1, 8, 9, output_as_list=True)
        john = ['\u2078 If we claim to be without sin, we deceive ourselves and the truth is not in us. ',
                '\u2079 If we confess our sins, he is faithful and just and will forgive us our sins and purify '
                'us from all unrighteousness.']
        self.assertEqual(john, text, 'Passage is incorrect')
    ...
    def test_get_chapter_list(self):
        text = bible_extractor.get_online_chapter('Ecclesiastes', 11, output_as_list=True)
    ...
    def test_get_chapters_list(self):
        text = bible_extractor.get_online_chapters('1 John', 1, 2, output_as_list=True)
        john = bible_extractor.get_online_chapter('1 John', 1, output_as_list=True) + \
            bible_extractor.get_online_chapter('1 John', 2, output_as_list=True)
    ...
    def test_get_passage_range_list(self):
        text = bible_extractor.get_online_passage_range('Ecclesiastes', 9, 18, 10, 1, output_as_list=True)
    ...
    def test_get_book_list(self):
        text = bible_extractor.get_online_book('Philemon', output_as_list=True)
    ...
    def test_get_passage_range_list_from_same_chapter(self):
        text = bible_extractor.get_online_passage_range('1 John', 1, 8, 1, 9, output_as_list=True)
        john = ['\u2078 If we claim to be without sin, we deceive ourselves and the truth is not in us. ',
                '\u2079 If we confess our sins, he is faithful and just and will forgive us our sins and purify '
                'us from all unrighteousness.']
        self.assertEqual(john, text, 'Passage is incorrect')
    ...
    def test_get_passages_list_with_stripped_whitespace(self):
        text = bible_extractor.get_online_passages('Ecclesiastes', 11, 6, 7, strip_excess_whitespace_from_list=True,
                                                   output_as_list=True)
    ...
    def test_get_yaml_passage_list(self):
        text = bible_yaml_extractor.get_yaml_passage('1 John', 1, 8, output_as_list=True)
        john = ['\u2078 If we claim to be without sin, we deceive ourselves and the truth is not in us. ']
        self.assertEqual(john, text, 'Passage is incorrect')

    def test_get_yaml_passages_list(self):
        text = bible_yaml_extractor.get_yaml_passages('1 John', 1, 8, 9, output_as_list=True)
        john = ['\u2078 If we claim to be without sin, we deceive ourselves and the truth is not in us. ',
                '\u2079 If we confess our sins, he is faithful and just and will forgive us our sins and purify '
                'us from all unrighteousness. ']
        self.assertEqual(john, text, 'Passage is incorrect')
    ...
    def test_get_yaml_book_list(self):
        text = bible_yaml_extractor.get_yaml_book('Philemon', output_as_list=True)
        eccl = bible_extractor.get_online_book('Philemon', output_as_list=True)
        # Results should be identical between the web and YAML extractor
        self.assertEqual(eccl, text, 'Passage is incorrect')
