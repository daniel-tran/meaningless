...
class UnitTests(unittest.TestCase):
    ...
    def get_static_file(filename):
        return f'./static/unit_tests_json_file_interface/{filename}'

    @staticmethod
    def get_temp_file(filename):
        return f'./tmp/unit_tests_json_file_interface/{filename}'
    ...
    def test_read_path_exceeds_windows_limit(self):
        ...
        self.assertRaises((FileNotFoundError, OSError), json_file_interface.read,
                          self.get_static_file(f'{filename}.json'))

    def test_write_path_exceeds_windows_limit(self):
        document = {'Disco': 7}
        filename = 'G' * 255
        self.assertRaises((FileNotFoundError, OSError), json_file_interface.write, f'./tmp/{filename}.json',
                          document)
